...
| `/api/reset`      | POST     | デフォルト画像に切り替え（画像保持） | `{"success":true,"message":"..."}`              |
| `/api/delete-all` | POST     | 全カスタム画像を完全削除             | `{"success":true,"message":"Deleted X images"}` |
| `/api/use-custom` | POST     | カスタム画像使用開始                 | `{"success":true}`                              |
| `/api/refresh`    | POST     | 画面強制更新                         | `{"success":true}`                              |

### 🔄 画像管理の改善

- **`/api/reset`**: カスタム画像を削除せず、デフォルト画像の表示に切り替え
- **`/api/delete-all`**: カスタム画像を完全削除（復元不可）
- **画像保持機能**: 誤操作による画像消失を防止
